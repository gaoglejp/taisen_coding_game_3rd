...
5. **Test coverage is growing (127 cases).** Vitest is wired up (PR #10).
   ...
   tested by mocking `@/lib/auth` / `@/lib/db` / `@/lib/audit`. plus player match-read routes (`/api/match/:id/{public,state,result,replay}`, PR #TBD).
