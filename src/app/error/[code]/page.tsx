...
export default function ErrorPage() {
  ...
  return (
    ...
    >
      ...
      {/* ============ State gallery — all error variants + admin ============ */}
      <StateGallery />
    </div>
  );
}
...
function StateGallery() {
  return (
    <section
      aria-label="エラー画面バリエーション"
      style={{ maxWidth: 1440, margin: "24px auto 32px", padding: "0 24px" }}
    >
      <div
        style={{
      ...
      >
        ...
        <p style={{ margin: 0, fontSize: 12, color: "var(--ink-soft)" }}>
          404 / 403 / 500 / メンテナンス / メンテナンス (進行中マッチあり) / 403 (管理 UI 版)
        </p>
      </div>
...
function StateShell({
  ...
  toneTag: string;
  toneTagBg?: string;
  toneTagBorder?: string;
  toneTagColor?: string;
  name: string;
  children: React.ReactNode;
  admin?: boolean;
  scopeMini?: React.ReactNode;
}) {
  return (
    <article
      style={{
    ...
    >
      <header
        style={{
          padding: "10px 14px",
          background: admin ? "var(--admin-bg, #1f2330)" : "var(--bg)",
          borderBottom: admin ? "1px solid #0f1117" : "1px solid var(--line)",
          color: admin ? "#e5e7eb" : "inherit",
          display: "flex",
          justifyContent: "space-between",
          alignItems: "center",
        }}
      >
        <span style={{ fontWeight: 700, fontSize: 12, color: admin ? "#f3f4f6" : "inherit" }}>
          {name}
        </span>
        <span
          style={{
            ...
            padding: "2px 6px",
            borderRadius: 4,
            background: admin ? "#2a2e3c" : toneTagBg ?? "#fff",
            border: admin ? "1px solid #3a3e4d" : `1px solid ${toneTagBorder ?? "var(--line)"}`,
            color: admin ? "#fff" : toneTagColor ?? "var(--ink-soft)",
          }}
        >
          {toneTag}
        </span>
      ...
      {scopeMini}
      <div style={{ padding: "24px 18px 20px", textAlign: "center" }}>{children}</div>
    </article>
  );
}
...
function State404() {
  return (
    <StateShell name="404 Not Found" toneTag="general" toneTagColor="var(--ink)">
      <div style={{ ...miniCode, color: "var(--ink)" }}>404</div>
      <div style={miniH}>ページが見つかりません</div>
      <div style={miniLead}>URL が変わったか、削除された可能性があります。</div>
      <div style={miniBtnRow}>
        <Link href="/dashboard" style={{ ...miniBtn, background: "var(--accent)", color: "#fff" }}>
          ⌂ ダッシュボード
        </Link>
        <Link href="/login" style={miniBtnGhost}>
          ログイン
        </Link>
      </div>
      <div style={miniTrace}>trc_a7f2_404_…</div>
    </StateShell>
  );
}
...
function State403() {
  return (
    <StateShell
      name="403 Forbidden"
      toneTag="general"
      toneTagBg="var(--accent-soft)"
      toneTagBorder="#fbd9a5"
      toneTagColor="#92400e"
    >
      <div style={{ ...miniCode, color: "var(--accent)" }}>403</div>
      <div style={miniH}>このページにアクセスできません</div>
      <div style={miniLead}>
        必要な権限がありません。
        <br />
        担当の管理者に依頼してください。
      </div>
      <div style={miniRoleRow}>
        <span>あなた:</span>
        <span style={rolePill("var(--p1-soft, #dbeafe)", "var(--p1-ink, #1e3a8a)", "#bfd5fa", true)}>
          ROOM_USER
        </span>
        <span>必要:</span>
        <span
      ...
      <div style={{ ...miniBtnRow, marginTop: 10 }}>
        <Link href="/dashboard" style={{ ...miniBtn, background: "var(--accent)", color: "#fff" }}>
          ⌂ ダッシュボード
        </Link>
        <Link href="/error/500" style={miniBtnGhost}>
          管理者に連絡
        </Link>
      </div>
    </StateShell>
...
function State500() {
  return (
    <StateShell
      ...
      toneTag="general"
      toneTagBg="var(--p2-soft, #fee2e2)"
      toneTagBorder="#fbb6b6"
      toneTagColor="var(--p2-ink, #7f1d1d)"
    >
      <div style={{ ...miniCode, color: "var(--danger)" }}>500</div>
      <div style={miniH}>予期しないエラーが発生しました</div>
      <div style={miniLead}>
        サーバ側の問題です。少し待ってから
        <br />
        再度お試しください。
      ...
      <div style={miniTrace}>trc_500_a7f29k3m</div>
    </StateShell>
  );
}
...
function StateMaint() {
  return (
    <StateShell
      name="メンテナンス"
      toneTag="scheduled"
      toneTagBg="var(--room-admin-accent-soft, #cffafe)"
      toneTagBorder="#a5f3fc"
      toneTagColor="var(--room-admin-accent-ink, #155e75)"
    >
      <div style={{ ...miniCode, color: "var(--room-admin-accent)", fontSize: 26 }}>🛠</div>
      <div style={miniH}>現在メンテナンス中です</div>
      <div style={miniLead}>
        サービス全体を一時停止しています。
        <br />
        ご不便をおかけしてすみません。
      ...
      <div style={miniCd}>
        <div style={miniCdLbl}>{"// 終了予定まで"}</div>
        <div style={miniCdVal}>01:23:45</div>
      </div>
      <div style={miniBtnRow}>
        <button style={{ ...miniBtn, background: "var(--room-admin-accent)", color: "#fff" }}>
          ↻ 状態を再取得
        </button>
      </div>
    </StateShell>
...
function StateMaintInterrupt() {
  return (
    <StateShell
      name="メンテナンス (進行中マッチあり)"
      toneTag="interrupt"
      toneTagBg="var(--room-admin-accent-soft, #cffafe)"
      toneTagBorder="#a5f3fc"
      toneTagColor="var(--room-admin-accent-ink, #155e75)"
    >
      <div style={{ ...miniCode, color: "var(--room-admin-accent)", fontSize: 26 }}>🛠</div>
      <div style={miniH}>対戦が中断されました</div>
      <div style={miniLead}>
        進行中の対戦は安全に中断され、
        <br />
        再開時に再戦のご案内をします。
      ...
      >
        <strong style={{ display: "block", fontSize: 11 }}>中断されたマッチ:</strong>
...
function State403Admin() {
  return (
    <StateShell
      name="403 Forbidden (管理 UI 版)"
      toneTag="admin"
      admin
      scopeMini={
        <div
        ...
        >
          <span
            ...
            style={{
              ...
              height: 14,
              borderRadius: 4,
              background: "var(--admin-accent-soft, #ede9fe)",
          ...
          <span>
            <strong style={{ color: "var(--ink)" }}>システム管理者として閲覧中。</strong>{" "}
            対戦操作はできません。
          </span>
        </div>
      }
    >
      <div style={{ ...miniCode, color: "var(--accent)" }}>403</div>
      <div style={miniH}>このルームには権限がありません</div>
      <div style={miniLead}>
        ROOM-2026-0288 はあなたが管理する
        <br />
        ルームではありません。
      </div>
      <div style={miniRoleRow}>
        ...
        <span
          style={rolePill(
            "var(--admin-accent-soft, #ede9fe)",
            "var(--admin-accent, #7c3aed)",
            "#ddd6fe",
            true
          )}
        >
      ...
      <div style={{ ...miniBtnRow, marginTop: 10 }}>
        ...
        <Link href="/admin" style={miniBtnGhost}>
          監査ログを見る
        </Link>
      </div>
    </StateShell>
...
const miniH: React.CSSProperties = {
  fontSize: 13,
  fontWeight: 700,
  marginBottom: 6,
  color: "var(--ink)",
...
const miniLead: React.CSSProperties = {
  fontSize: 11,
  color: "var(--ink-soft)",
  lineHeight: 1.5,
  marginBottom: 12,
...
const miniBtnRow: React.CSSProperties = {
  display: "flex",
  justifyContent: "center",
  gap: 6,
  flexWrap: "wrap",
...
const miniTrace: React.CSSProperties = {
  marginTop: 10,
  fontFamily: "JetBrains Mono, monospace",
  fontSize: 9.5,
  color: "var(--ink-soft)",
...
const miniRoleRow: React.CSSProperties = {
  marginTop: 10,
  background: "var(--bg)",
  border: "1px solid var(--line)",
  borderRadius: 7,
...
const miniCd: React.CSSProperties = {
  background: "linear-gradient(180deg, #f0fdfa, #ecfeff)",
  border: "1px solid #99f6e4",
  borderRadius: 8,
  padding: 8,
...
const miniCdLbl: React.CSSProperties = {
  fontFamily: "JetBrains Mono, monospace",
  fontSize: 9,
  color: "var(--room-admin-accent-ink, #155e75)",
  textTransform: "uppercase",
...
const miniCdVal: React.CSSProperties = {
  fontFamily: "JetBrains Mono, monospace",
  fontSize: 22,
  fontWeight: 800,
  color: "var(--room-admin-accent-ink, #155e75)",
